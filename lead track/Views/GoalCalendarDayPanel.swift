...
/// The card under the grid for a tapped day: each judged series that day as
/// a row — icon, name, the actual value against its goal, and the verdict.
/// A filtered calendar shows its one series; a tallied calendar lists every
/// metric that carried a goal (or logged effort) that day.
...
extension GoalCalendarDayPanel {
    /// One panel row's resolved facts.
